 ...
 const float b=83.2073,a=11.003; //(adc+b)/a for db founction
...
start:while(1)
  {
   ...
   Rx=0;
    if (Button(&PORTC, 3, 1, 0))        //buttone calcule de R
    {
      oldstate = 1;
    }
    ...
    if (Button(&PORTc, 4, 1, 0))   //buttone calcule de F
    {
     oldstate1=1;
    }
    ...
    if (Button(&PORTc, 5, 1, 0))  //  buttone calcule de db
    {
    oldstate2=1;
    }
    else if(Button(&PORTc, 5, 1, 1)&&(oldstate2))
    ...
    while(1)
      {
        ...
        // delay_ms(100);
         // if(db>60){portc.rc7=1;}else{portc.rc7=0;}
